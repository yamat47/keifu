...
- **`dist/` に約240個の woff2 が出る**（ウェイト400のみで 7.6 MB）。
  Cloudflare Static Assets の上限は 20,000 ファイル / 1ファイル 25 MiB なので収まる。
  閲覧者が取るのはそのうち数個
- **`@font-face` の CSS が約220 KB になる。** `unicode-range` の羅列の分。
  圧縮は効くが、CSS バンドルの中では最も大きい部分になる
